      SUBROUTINE ZBQRU_APPQ( SIDE, TRANS, M, N, K, V, LDV, TAU, C, LDC,
...
*  =======
*
*  Applies the unitary factor Q (or its transpose) generated by the
*  routine ZBQRU.
...
*  V(:,i), and tau in TAU(i). See also ZBQRU.
*
*  =====================================================================
*
*     .. Parameters ..
...
*     NB = panel block size
      NB = ILAENV( 1, 'ZGEQRF', ' ', ROWV, UPD, -1, -1 )
      NB = MIN( NB, NBMAX )
C      PRINT*, 'NB = ', NB
...
*
            DO 20  J = 1, K, NB
               JB = MIN( K-J+1, NB )
               IF ( LBLOCK ) THEN
                  CALL ZBQRU_LARFT( ROWV, JB, V(1,J), LDV, TAU(J),
     $                              T, LDT )
                  CALL ZBQRU_LARFB( 'Left', 'No Transpose', M, N, JB,
     $                              V(1,J), LDV, T, LDT, C(J,1),
     $                              LDC, D, LDD, WORK, UPD )
               ELSE
                  DO 10 I = J, J+JB-1
                     CALL ZBQRU_LARF( 'Left', ROWV, N, V( 1, I ), 1,
...
*
            DO 40  J = ( ( K-1 ) / NB )*NB + 1, 1, -NB
               ...
               IF ( LBLOCK ) THEN
                  CALL ZBQRU_LARFT( ROWV, JB, V(1,J), LDV, TAU(J),
     $                              T, LDT )
                  CALL ZBQRU_LARFB( 'Left', 'Conjugate transpose', M, N,
     $                              JB, V(1,J), LDV, T, LDT, C(J,1),
     $                              LDC, D, LDD, WORK, UPD )
               ELSE
                  DO 30 I = J+JB-1, J, -1
                     CALL ZBQRU_LARF( 'Left', ROWV, N, V( 1, I ), 1,
...
*
            DO 100  J = 1, K, NB
               ...
               IF ( LBLOCK ) THEN
                  CALL ZBQRU_LARFT( ROWV, JB, V(1,J), LDV, TAU(J),
     $                              T, LDT )
                  CALL ZBQRU_LARFB( 'Right', 'No Transpose', M, N, JB,
     $                              V(1,J), LDV, T, LDT, C(1,J),
     $                              LDC, D, LDD, WORK, UPD )
               ELSE
                  DO 90 I = J, J+JB-1
                     CALL ZBQRU_LARF( 'Right', M, ROWV, V( 1, I ), 1,
     $                                TAU( I ), C( 1, I ), 1,
     $                                D, LDD, WORK )
   90             CONTINUE
               END IF
         ...
         ELSE
            DO 120  J = ( ( K-1 ) / NB )*NB + 1, 1, -NB
               ...
               IF ( LBLOCK ) THEN
                  CALL ZBQRU_LARFT( ROWV, JB, V(1,J), LDV, TAU(J),
     $                              T, LDT )
                  CALL ZBQRU_LARFB( 'Right', 'Conjugate Transpose', M,
     $                              N, JB, V(1,J), LDV, T, LDT, C(1,J),
     $                              LDC, D, LDD, WORK, UPD )
               ELSE
                  DO 110 I = J+JB-1, J, -1
                     CALL ZBQRU_LARF( 'Right', M, ROWV, V( 1, I ), 1,
     $                                DCONJG( TAU( I ) ), C( 1, I ), 1,
     $                                D, LDD, WORK )
  110             CONTINUE
               END IF
  120       CONTINUE
