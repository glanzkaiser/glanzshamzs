...
int main(int argumentCount, char * argumentValues[])
{
	...
	if (skin)
	{
		...
		gui::IGUIFont* font = guienv->getFont("/opt/hamzstlib/Physics/irrlicht-1.8.5/media/fontlucida.png");
		if(font)
			skin->setFont(font);
	}
...
*/
	for(f32 zoneX = -100.f; zoneX <= 100.f; zoneX += 50.f)
		for(f32 zoneY = -60.f; zoneY <= 60.f; zoneY += 60.f)
		{
			...
			billboard->setMaterialTexture(0, driver->getTexture("/opt/hamzstlib/Physics/irrlicht-1.8.5/media/particle.bmp"));
